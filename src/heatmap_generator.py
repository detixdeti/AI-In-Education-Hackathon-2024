...
class HeatmapGenerator:
    ...
    def plot_heatmap(self, group_by_column, score_bin_column, title, cmap='viridis', figsize=(10, 8)):
        """
        Plots a heatmap showing the frequency of scores for a specific grouping.
        ...
        sns.heatmap(frequency_table, annot=True, cmap=cmap, linewidths=0.5, fmt='d')
        plt.title(title)
...
if __name__ == "__main__":
    ...
    # Plot heatmaps for different groupings
    heatmap_gen.plot_heatmap(group_by_column='race/ethnicity', score_bin_column='math_score_bins', 
                             title='Frequency of Math Scores by Ethnicity')

    heatmap_gen.plot_heatmap(group_by_column='gender', score_bin_column='math_score_bins', 
                             title='Frequency of Math Scores by Gender')

    heatmap_gen.plot_heatmap(group_by_column='parental level of education', score_bin_column='math_score_bins', 
                             title='Influence of Parental Education on Math Scores')
